...
st.markdown(
    ...
    **Workflow**: Run *Portfolio Explorer* first to build a panel for your chosen managers and hedging setup. 
    This page will pick that up from session. If not found, it will parse the workbook at `data/Manager Track Records v2.xlsx` as a fallback (GBP base).
    """
)
...
edited = st.data_editor(
    ...
    column_config={
        ...
        'Weight': st.column_config.NumberColumn(format='%.4f', help='Only used if Lock=True; also used as initial guess')
    },
...
if btn:
    ...
    def annualize_std(rm):
        return rm.std(ddof=0) * math.sqrt(12)

    def cagr_from_series(rm):
        if rm.empty:
            return np.nan
        cg = (1.0 + rm).prod()
        yrs = len(rm) / 12.0
        return cg**(1/yrs) - 1 if yrs > 0 else np.nan

    def max_drawdown_from_series(rm):
        cum = (1.0 + rm).cumprod()
    ...
    if minimize is None:
        st.warning("SciPy not available. Using a heuristic search (coarse grid) — consider adding scipy to requirements for best results.")
        # Simple heuristic: project w0 into bounds, normalize; then do a basic random local search
        w = np.clip(w0, lb, ub)
        if w.sum() == 0:
        ...
        res = minimize(lambda w: obj_value(w), w0, method='SLSQP', bounds=bounds, constraints=cons, options={'maxiter': 1000})
        if not res.success:
    ...
    # Output weights
    out = pd.DataFrame({'Manager': cols, 'Weight': w_opt})
    st.subheader("Optimal Weights")
    st.dataframe(out.style.format({'Weight': '{:.2%}'}), use_container_width=True)

    # Build optimized portfolio
    port = (panel[cols] @ w_opt).dropna().rename('Portfolio (Optimized)')

    # --- Analysis (match Portfolio Explorer visuals) ---
    st.subheader("Analysis")
    chart = st.radio("Chart", ["Summary","Cumulative","Drawdown","12M Return","12M Vol","Monthly Bars","Correlation","Year×Month"], index=0, horizontal=True)
    ...
    if chart == "Summary":
        ...
        fig.add_trace(go.Scatter(x=cum_port.index, y=cum_port.values, name='Optimized', line=dict(width=3, color='black')))
        # Optionally overlay constituents
        for m in cols:
            s = panel[m].dropna()
            fig.add_trace(go.Scatter(x=s.index, y=(1+s).cumprod(), name=m, line=dict(width=1), opacity=0.4))
        fig.update_layout(title='Cumulative Growth of £1', hovermode='x unified', legend=dict(orientation='h'), yaxis_title='Value (£)', xaxis_title='Date', margin=dict(l=40,r=20,t=60,b=40))
        ...
        fig.update_layout(title='Portfolio Drawdown', hovermode='x unified', xaxis_title='Date', yaxis_title='Drawdown', margin=dict(l=40,r=20,t=60,b=40))
        ...
        fig_bar = px.bar(dfb, x='Month', y='Monthly Return', title='Portfolio Monthly Returns', color='Monthly Return', color_continuous_scale='RdYlGn')
        fig_bar.update_yaxes(tickformat='.0%')
        fig_bar.update_layout(hovermode='x unified', margin=dict(l=40,r=20,t=60,b=40))
        st.plotly_chart(fig_bar, use_container_width=True)

    elif chart == "Correlation":
        filt = panel[cols].copy()
        ...
        if len(valid_cols) >= 2:
            ...
            fig_corr = px.imshow(corr.round(2), text_auto=True, color_continuous_scale='RdBu_r', zmin=-1, zmax=1, title='Correlation (monthly returns)')
        ...
        if piv.empty:
            ...
            fig_hm.update_layout(title='Year × Month (Portfolio monthly returns)', xaxis_title='Month', yaxis_title='Year', coloraxis_colorbar=dict(title='Return', tickformat='.0%'), margin=dict(l=60,r=20,t=60,b=60))
