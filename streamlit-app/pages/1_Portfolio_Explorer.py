...
default_sel = manager_options[: min(4, len(manager_options))]
chosen = st.sidebar.multiselect("Managers", manager_options, default=default_sel)
...
# Dynamic weight sliders (allow negatives)
weights = {}
if chosen:
...
# Segmented control fallback for older Streamlit versions
...
if run and chosen:
    ...
    w = pd.Series([weights[m] for m in chosen], index=chosen, dtype=float)
    if normalize and w.sum() != 0:
        w = w / w.sum()
    port = (panel[chosen] * w).sum(axis=1).dropna().rename("Portfolio")
    ...
    if chart == "Summary":
        ...
        st.dataframe(df_stats.style.format({
            "Ann. Return":"{:.2%}","Ann. Vol":"{:.2%}","Sharpe (rf=0)":"{:.2f}","Max Drawdown":"{:.2%}","Calmar":"{:.2f}"
        }))
        ...
        if piv.empty:
            ...
            st.dataframe(piv.style.format("{:.1%}").background_gradient(cmap="RdYlGn", axis=None))
else:
